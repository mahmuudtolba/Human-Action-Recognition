...
def action_recognation(URL_recognation):
    """
                ...
                if d > 250 :
                    ...
                    if len(sequence) == 50:
                        ...
                        if np.unique(predictions[-40:])[0]==np.argmax(res) and res[np.argmax(res)] > threshold and action_prediction in order[0]: 
                            ...
                            #print(order[0])
                            #if res[np.argmax(res)] > threshold and actions[np.argmax(res)] in order[0]: 
                            predictions = []
                            sequence = []
                            time.sleep(1)
                            tmp = order.pop(0)
...
def face_expression(URL_expression):
    # Read feed
    labels = []
    while cap.isOpened():
        ...
        if faces:
            ...
            if len(labels) == 10 :
                expression = max(set(labels) , key = list.count)
